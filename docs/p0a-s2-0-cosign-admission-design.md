...
**状态**: 设计定案 — 关键项已实证：chart 0.10.6 自带 2 CRD、2 Validating+2 Mutating webhook 拓扑、failurePolicy(line 44)/no-match(configData line 17) value、arm64 镜像 manifest 均已 `helm template --include-crds`/`docker manifest inspect` 核实；**glob 匹配形式**已用 policy-controller `glob.Compile` 源码逐字复算定案为 `@sha256:**`（§6，复算脚本入库）。**仅**跨身份真 fixture 动态负测 + webhook 端到端 admission smoke-test + init/ephemeral 动态正反测留 **staging e2e**（本地不可伪造 Fulcio OIDC / 需真 apiserver-webhook 调用）；其余静态事实（ephemeral 子资源 rules、multi-source 渲染集合）实现 PR 前从固定 chart 渲染定案。待用户复核 → 落实现。
**日期**: 2026-07-19
...
**实证方法**：policy-controller 的 `glob.Compile` 是确定性正则翻译（`.`→`\.`、`**`→`.*`、`*`→`[^/]*`、`^...$` 全串锚定），匹配对象是 `name.ParseReference(image).Name()`（规范化后完整引用：`docker.io`/短名→`index.docker.io`，digest 保留 `@sha256:...`）。用该源码逐字复算四个候选 glob × 六种镜像引用（复算脚本存 `scripts/image-pin/testdata/glob_match_check.go.txt`）：
...
- **✅ 采用 `index.docker.io/wontlost/aster-api@sha256:**`**（migrate 同形 `.../aster-cloud-migrate@sha256:**`）：三种等价 digest 引用（`docker.io/`、`index.docker.io/`、短名）全命中、前缀不误纳、别 registry 不漏、且**只匹配 digest 形式**。因部署本就 by-digest（kustomize transformer→`@sha256:`），完全对齐。
- **★残留 gap（诚实记录）**：`@sha256:**` **不匹配 tag 形式**（如 transformer 失效 fallback 到 `:jvm-latest`）→ 该 tag 镜像落 `no-match-policy: allow` 被放行，逃过 enforce。缓解：现有 `verify-rendered-by-digest.yml` 已守「受控镜像不得回落 floating tag」；S2-0 层面可加第二个 CIP（`...aster-api:**` glob，任何 tag 形式→无有效签名身份→拒）**强制只准 digest 形式**。**实现时决定是否加此「拒 tag」CIP**（推荐加，闭合 gap）。
...
3. **★绕过面矩阵（P1，Codex——遗漏则运行时注入绕过）**：分别测 **Pod / Deployment/ReplicaSet / Job/CronJob / init container / ephemeral container（`kubectl debug --image=<未签受控仓镜像>`）/ controller 创建的 Pod**——每种都用「匹配 glob 的未签镜像」跑，证明全部被拦。★**静态事实先定案（不留 staging）**：从 chart 0.10.6 渲染的 webhook `rules.resources`（`helm template ... | grep -A rules`）确认是否含 **`pods/ephemeralcontainers`** 子资源——这是**可本地从固定 chart 渲染定案的静态事实**，实现 PR 前写回本文档；若**不含** → ephemeral container 是运行时注入绕过路径 → 补策略（额外 webhook rule）或明确列为阻塞。init container 同理从渲染 rules + PodSpec 验证逻辑静态确认，动态正反测在 staging 补。
...
5. **glob 匹配矩阵（已本地定案，§6）**：用 `glob.Compile` 源码逐字复算（`scripts/image-pin/testdata/glob_match_check.go.txt`）已证 `@sha256:**` 只命中三等价 digest 引用、不误纳前缀/别 registry/tag。实现时用 `policy-tester`（同 chart 版本）真跑复验一遍。
