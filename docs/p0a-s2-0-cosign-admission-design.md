...
**状态**: 设计定案（Codex 交叉审查 7 轮通过，93/100）。验证分三层：
- **设计已定事实（源码/渲染实证）**：chart 0.10.6 自带 2 CRD、2 Validating+2 Mutating webhook 拓扑、failurePolicy(value line 44)/no-match(configData line 17)、arm64 镜像 manifest（`helm template --include-crds`/`docker manifest inspect`）；**glob=`@sha256:**`**（v0.13.1 `glob.Compile` 源码复算 + 可复现 Go test，§6）；**ephemeral/init 被覆盖**（`cmd/webhook/main.go:184` 源码，§7）；**tag→digest 正常路径无逃逸 + 不可解析 tag TOCTOU 已由 tag-fail CIP 闭合**（`resolveDigest` 失败 `continue` 保留 tag，`validator.go:1079`；`static:{action:fail}` 是合法 authority，`common/validation.go:46`；§6）。CIP 总数 = 4（2 digest-verify + 2 tag-fail）。
...
**为什么仍要做**：任何层3 都要绑「实际跑的 image」；若运营方/漂移能让集群跑任意未签 digest，层3 的 image binding 前提不成立。★**准确定位（Codex P1）**：S2-0 是 **S2-1 的必要控制之一**，**不是**「地基已牢」——它保证「**opt-in namespace 的受控两仓范围内、webhook 健康时**只准入受信来源镜像」，但**不绑 pod UID / node / container ID / 实际响应 / 特定执行实例**（那些是 S2-1 β 的 execution binding）。S2-0 有独立价值（缩小可运行镜像集合）+ 为 S2-1 收窄攻击面，但单独不构成 runtime provenance。
...
│  单个 Application (multi-source: Helm chart + 四个 CIP 清单)  │
...
│   mode: enforce                                              │
│                                                              │
│ + 每仓再加 tag-fail CIP(闭 TOCTOU, §6):                       │
│   wontlost-aster-api-reject-tag / -migrate-reject-tag        │
│     images: glob .../aster-api:**   (仍为 tag 形式)          │
│     authorities: static { action: fail }   mode: enforce     │
│   → 不可解析 tag 保持 tag 形式 → 无条件拒(防 admission 后拉未签) │
│   CIP 总数=4 (2 digest-verify + 2 tag-fail)                   │
└─────────────────────────────────────────────────────────────┘
        │  灰度双闸(见 §4):
...
★**P0-1 修正（Codex 退回）**：**每个受控镜像必须独立一个 CIP，每个 CIP 只含该镜像对应的精确 signer 身份**。policy-controller 语义：多个匹配 CIP 之间是 **AND**，**同一 CIP 内多身份是 OR**。若把 2 镜像 + 2 身份塞进一个 CIP，实际授权 = `{aster-api, migrate} × {deploy.yml, ci.yml}` 笛卡尔积 → `aster-api` 被 `aster-cloud/ci.yml` 签也能过，**违反信任根「每镜像唯一合法来源」**。故拆成两个 digest-verify CIP（每仓一个；另有两个 tag-fail CIP，见 §6 → 共 4 CIP）。★用**精确 `subject`（非 `subjectRegExp`）**——身份是确定字符串；若必须用正则须 `^...$` 锚定 + 测试转义。★镜像用 `index.docker.io/...`（policy-controller 对 Docker Hub 规范化到 `index.docker.io`，见 §6 实测）+ glob 收紧防前缀误纳（见 §6）。
...
2. **四个 CIP 文件**（新，置于 `policies/` 子目录，全标 `sync-wave: "2"`）——每受控仓 **2 个 CIP**：
   - **digest-verify CIP**（P0-1，每镜像独立 + 精确 `subject`）：`cluster-image-policy-aster-api.yaml`（glob `index.docker.io/wontlost/aster-api@sha256:**`，keyless subject=`.../aster-cloud/aster-api/.github/workflows/deploy.yml@refs/heads/main`）+ `cluster-image-policy-aster-cloud-migrate.yaml`（glob `.../aster-cloud-migrate@sha256:**`，subject=`.../aster-cloud/aster-cloud/.github/workflows/ci.yml@refs/heads/main`）。
   - **tag-fail CIP**（P0 TOCTOU 闭合，§6）：`cluster-image-policy-aster-api-reject-tag.yaml`（glob `.../aster-api:**`，`static: { action: fail }`）+ migrate 同形——拒任何仍为 tag 形式（不可解析 tag）的受控仓镜像。
   ★`no-match-policy` ConfigMap **不在这些文件**（归 chart/controller wave 0，§3b）。
...
- **wave 2**（Git source `policies/`，显式标 `argocd.argoproj.io/sync-wave: "2"`）：**四个 CIP**（2 digest-verify + 2 tag-fail，§6 TOCTOU 闭合）。ArgoCD 等 wave 0 全部 `Healthy`（含 CRD `Established` + controller Deployment Ready）才推进 wave 2 → CIP apply 时 CRD 必已 established。
...
**ConfigMap 归属**：`config-policy-controller`（no-match-policy）**由 chart/controller 部分（wave 0）管理**（它是 controller 运行配置），`policies/` 目录只含**四个 `ClusterImagePolicy`**。唯一所有者，无歧义。
...
└── policies/                   # 内层 Application multi-source 的 Git source 只指向这里
    ├── kustomization.yaml
    ├── cluster-image-policy-aster-api.yaml               # digest-verify (keyless)
    ├── cluster-image-policy-aster-api-reject-tag.yaml    # tag-fail (static:fail)
    ├── cluster-image-policy-aster-cloud-migrate.yaml     # digest-verify (keyless)
    └── cluster-image-policy-aster-cloud-migrate-reject-tag.yaml  # tag-fail (static:fail)
```
...
4. 删 controller、**四个 CIP**（2 digest-verify + 2 tag-fail）、2 个 CRD（`clusterimagepolicies`/`trustroots`）。
...
**★漂移风险 + 缓解**：CIP（四份 YAML；每受控仓一个 digest-verify + 一个 tag-fail）与 `allowed-images.yaml` 手写 → 可能漂移。缓解：
- **首版**：CIP 注释显式指向 `allowed-images.yaml` 为权威源 + CI 校验脚本 `scripts/image-pin/verify-cip-sync.sh` 守**完整四 CIP 契约漂移**（数量 N 仓→2N、repository、glob、mode、authority 类型、signer identity——见 §7；且 digest CIP 须 `images` 恰一项、`authorities` 恰一项、`keyless.identities` 恰一项，**禁止额外 identity 经 OR 扩权**）。★注意：脚本防 CIP 契约漂移，**不弥合上面的 source-SHA/freshness 语义差异**（那是设计上的有意不复制，非漂移）。
...
- **结论（正常路径）：可解析 tag → Mutating 解析为 digest → Validating 命中 digest CIP → 被强制验签。**

★★**但存在真 TOCTOU 绕过（Codex 第 5 轮 P0，源码确认——我上一版说「拉不起来所以无害」是错的）**：查 v0.13.1 `resolvePodSpec`（`validator.go:1079-1081`）——**`resolveDigest` 失败时是 `continue`（保留原 tag），mutation best-effort 不 return error**；`ResolvePod` 也不返回 error（`validator.go:1016`）。故：
```
提交受控仓「不可解析的 tag」（当前不存在/registry 暂不可达/认证暂失败）
  → Mutating resolveDigest 失败 → continue 保留 repo:tag
  → Validating: ref.Name()=repo:tag（含 :latest 默认）不匹配 @sha256:** digest CIP
  → no-match-policy: allow → Pod 被 API server 接受
  → kubelet 持续重试拉取；攻击者随后向该 tag 推未签镜像 / registry 恢复
  → kubelet 拉取并运行（不再过 admission）= 绕过
```
「admission 时拉不起来」≠「未来不会运行」——admission 后状态可变，典型 TOCTOU。

★**闭合方案（必须做，非「实现时决定」）——每受控仓加第二个 tag-fail CIP**（`static: { action: fail }` 是 v0.13.1 合法 authority，`StaticRef.Action ∈ {fail,pass}`，`common/validation.go:46` 确认）：
```yaml
# 除 wontlost-aster-api（digest CIP）外，再加：
apiVersion: policy.sigstore.dev/v1beta1
kind: ClusterImagePolicy
metadata: { name: wontlost-aster-api-reject-tag }
spec:
  images:
    - glob: index.docker.io/wontlost/aster-api:**   # 任何仍为 tag 形式的
  authorities:
    - static: { action: fail }                       # 无条件拒
  mode: enforce
```
**语义（源码推定 + staging 必验）**：可解析 tag → mutation 后变 digest → `ref.Name()` 丢 tag → 只命中 digest CIP（不再命中 tag CIP）；**不可解析 tag → 保持 `repo:tag` → 命中 tag-fail CIP → 无条件拒**（闭 TOCTOU）；已提交 digest → 只命中 digest CIP；别仓 → `no-match-policy: allow` 灰度。**★staging 必验**：mutation 后对象确实不再命中 tag CIP（否则合法镜像被误拒）。故 **CIP 总数 = 4**（2 digest-verify + 2 tag-fail）。
...
1. **CIP schema + keyless 语义 + 灰度四态**：本地 `kind`/`k3d`（arm64 mac 原生）→ `helm install policy-controller sigstore/policy-controller -n cosign-system` + `no-match-policy: allow` → apply **四个 CIP**（2 digest-verify + 2 tag-fail）→ 测试 namespace 贴 `policy.sigstore.dev/include=true` → 验证：(a) 匹配 digest glob 的**已签**镜像=通过；(b) 匹配 digest glob 的**未签/错身份**镜像=被拒（enforce）；(c) **不匹配任何 CIP** 的镜像（模拟 sidecar）=放行（`no-match-policy: allow`）；(d) **未贴标签** namespace 任意镜像=放行（opt-in）。
2. **★P0-1 交叉授权负测（★Codex——身份不可本地伪造，须用真 fixture）**：目标 = 证明「`aster-cloud/ci.yml` 身份签的 `aster-api` 镜像被拒」。但**本地 kind/k3d 不能伪造 Fulcio OIDC 身份**。故采用：(a) **静态**断言两 digest-verify CIP 的 image↔subject 一一对应（`verify-cip-sync.sh` 覆盖）；(b) **动态**用 policy-tester + **固定证书/签名 fixture**（预先由受控 GitHub Actions 测试 workflow 生成的 cross-signed fixture，或用已存在的错身份签名测试 digest）→ 证明错身份被拒。**不把「本地伪造身份」写成可执行步骤**；真跨身份动态负测在 staging 用真实 fixture 补。
...
4. **★tag→digest 语义 + TOCTOU 闭合（Codex 第 4/5 轮 P0，源码定论）**：正常路径 Mutating 解析 tag→digest→Validating 命中 digest CIP 验签；**不可解析 tag 有 TOCTOU 绕过**（`resolveDigest` 失败 `continue` 保留 tag→no-match allow→接受→后续 kubelet 拉未签），已加 **tag-fail CIP** 闭合（§6）。**staging 必验四态**：(a) 已签可解析 tag → mutation 后 digest、digest CIP 通过；(b) 未签可解析 tag → digest CIP 验签失败被拒；(c) **不可解析 tag（不存在的 tag）→ tag-fail CIP 无条件拒**（闭 TOCTOU 关键）；(d) mutation 后的 digest 对象**不再命中 tag CIP**（否则合法镜像误拒）。
5. **glob 匹配矩阵（已本地定案 + 可复现测试，§6）**：`scripts/image-pin/testdata/globcheck/`（固定 v0.13.1 `glob.Compile` 逻辑的表驱动 Go test，`go test -v ./...` 已绿，含 README）证 `@sha256:**` 命中三等价 digest 引用、不误纳前缀/别 registry/tag；**tag-fail CIP `:**` 命中仍为 tag 形式的镜像、不误纳 digest**（TOCTOU 闭合证据）；覆盖两仓。staging 用 `policy-tester`（同 chart appVersion）真跑复验。
6. **信任根一致性（★Codex——守门须覆盖完整四 CIP 契约，防 tag-fail 被误删/漂移）**：`scripts/image-pin/verify-cip-sync.sh` 断言**完整四 CIP 契约**：
   - 每个信任根 repository **恰有一个 digest-verify CIP**：glob==`index.docker.io/<repo>@sha256:**`，keyless issuer/subject == `allowed-images.yaml` 派生值；
   - 每个 repository **恰有一个 tag-fail CIP**：glob==`index.docker.io/<repo>:**`，authority 唯一且为 `static.action: fail`；
   - **四份 CIP 均 `mode: enforce`**；
   - **不允许额外受控 repository 或缺失任一 CIP**（数量与信任根严格对应：N 仓 → 2N CIP）。
   改任一字段（含删 tag-fail、glob 漂移、mode 变更）→ 报错。
...
8. **ArgoCD 清单合法性 + multi-source 渲染集合**：`kustomize build` + `kubeconform`/`kubeval` 静态校验；★确认内层 Git source 指向 `policies/` 只渲染**四个 CIP**（不含 `application.yaml`，防递归认领，§3b 目录结构）。
